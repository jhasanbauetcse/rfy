...
let leases = [];
let payments = []; // NEW: State for payments
let currentPaymentData = {}; // NEW: To hold data for gateway
...
function renderQuickActions() {
  ...
  leases.forEach(async (l) => {
    ...
    el.className = "qa";
    let propertyIdentifier = await getPropertyIdentifier(l.propertyId);
    // Store propertyIdentifier in the lease object for later use
    l.propertyIdentifier = propertyIdentifier;
...
async function qaHandler(e) {
  ...
  // Property identifier might already be on leaseData if renderQuickActions completed
  if (!leaseData.propertyIdentifier) {
  ...
  list.innerHTML = payments
    ...
    <div class="list-item payment-item">
      ...
      <div class="payment-details">
        <span class="muted">Paid on: ${fmtDate(p.paymentDate)}</span>
...
saveProfileBtn.addEventListener("click", async () => {
  ...
  if (!updates.fullName || !updates.email) {
    showMessage("Error", "Full Name and Email are required.");
    return;
  }
...
const virtualGatewayModal = $("#virtualGatewayModal");
...
function openPayDialog(lease) {
  ...
  if (!nextDueDate) {
    nextDueDate = new Date(); // default to today if no due date
  }
...
$("#payForm").addEventListener("submit", async (e) => {
  ...
  const forDate = $("#payForDate").value;
  ...
  // Store data for the gateway
  currentPaymentData = {
    amount: amount,
    forMonthYear: fmtMonthYear(forDate),
    lease: payDialogLease,
  };

  // Populate and open gateway
  $("#gatewayProperty").textContent = payDialogLease.propertyIdentifier;
  $("#gatewayAmount").textContent = fmtMoney(amount);
  $("#fakeName").value = tenantProfile.fullName || "Tenant";

  payDialog.close();
  virtualGatewayModal.showModal();
});

$("#cancelGateway").addEventListener("click", () => {
  virtualGatewayModal.close();
  currentPaymentData = {}; // Clear data
});

// Virtual Gateway Form Submission
$("#virtualGatewayForm").addEventListener("submit", async (e) => {
  e.preventDefault();
  const btn = $("#submitGatewayPayment");
  btn.disabled = true;
  ...
  setTimeout(async () => {
    try {
      const lease = currentPaymentData.lease;
      const paymentRecord = {
        tenantId: currentUser.uid,
        tenantName: tenantProfile.fullName || "N/A",
        landlordId: lease.landlordId,
        propertyId: lease.propertyId,
        propertyIdentifier: lease.propertyIdentifier,
        amount: currentPaymentData.amount,
        forMonthYear: currentPaymentData.forMonthYear,
      ...
      showMessage(
        "Payment Successful",
        `Your payment of ${fmtMoney(
          currentPaymentData.amount
        )} has been processed.`
      );

      currentPaymentData = {}; // Clear temp data
...
$("#maintForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    $("#maintForm").reset();
    loadMaintenance();
    showMessage("Success", "Your maintenance request has been submitted.");
  } catch (err) {
    console.error(err);
...
$("#newMaintBtn").addEventListener("click", () => {
  if (!leases.length) {
    showMessage("No Lease", "No leases available to submit a request for.");
    return;
  }
    ...
    if (propertyId) {
      try {
        ...
        if (propDoc.exists) {
          ...
          showMessage("Error", "Property details not found.");
        }
      } catch (error) {
        console.error("Error fetching property details:", error);
        showMessage("Error", "Could not fetch property details.");
      }
    }
  }
});
